...
def download_video_file(url: str, filename: str = "temp_video_recipe.mp4"):
    ...
    if os.getenv("PROXY_URL"):
        print(f" -> Using proxy for download: {os.getenv('PROXY_URL')}")
...
def get_video_metadata(url: str):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not info:
                print("   -> yt-dlp returned no info. Trying API fallback...")
                return _youtube_api_fallback(url)
        ...
        print(f"   -> yt-dlp exception: {e}. Trying API fallback...")
        return _youtube_api_fallback(url)
            
def _youtube_api_fallback(url):
    """Helper for YouTube API + Transcript API fallback."""
    # 1. Extract ID manually
    vid_id = extract_video_id.invoke(url)
    print(f"   -> Manual ID extraction: {vid_id}")
    
    if vid_id and (os.getenv("YT_API_KEY") or os.getenv("GOOGLE_API_KEY")):
        from googleapiclient.discovery import build
        try:
            api_key = os.getenv("YT_API_KEY") or os.getenv("GOOGLE_API_KEY")
            youtube = build('youtube', 'v3', developerKey=api_key)
            ...
            print(f"      -> YouTube API fallback failed: {api_e}")
